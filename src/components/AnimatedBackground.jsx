...
export default function AnimatedBackground() {
  ...
  useEffect(() => {
    ...
    const orbs = [
      { x: window.innerWidth * 0.2, y: window.innerHeight * 0.25, r: 380, color: COLORS.primary, vx: 0.25, vy: 0.15, phase: 0 },
      { x: window.innerWidth * 0.8, y: window.innerHeight * 0.6, r: 320, color: COLORS.secondary, vx: -0.2, vy: 0.28, phase: 2 },
      { x: window.innerWidth * 0.5, y: window.innerHeight * 0.8, r: 260, color: COLORS.accent, vx: 0.18, vy: -0.22, phase: 4 },
    ];
    ...
    const draw = () => {
      ...
      particles.forEach((p, i) => {
        ...
        const hex = Math.floor(Math.max(0, Math.min(1, p.opacity)) * 255).toString(16).padStart(2, '0');
        ctx.beginPath();
        ...
        if (DRAW_LINES) {
          for (let j = i + 1; j < particles.length; j++) {
            ...
            if (distP < 90) {
              ...
              const hexLine = Math.floor(alpha * 255).toString(16).padStart(2, '0');
              ctx.beginPath();
              ctx.moveTo(p.x, p.y);
              ctx.lineTo(q.x, q.y);
